<?php
	include("define_staticpage.php");

	define("SITEMAIL", "info@seanmitchellagencies.com.au");
	define("SITEPHONE", "1300 885 425");
	define("ADMINFOLDER", "apanel");
	define("CTABLE_ADMIN", "admin"); // for admin access
	define("SESS_PRE", "FOX");
	define("CUR", "$");

	define("ISMAIL", true);
	define("TAX_RATE", 10);  // in percentage (10%)

	define('SITEADDRESS', "2/11 Bent Street <br />St Marys NSW 2160 <br />Australia");

	define("PAYPAL_URL","");
	define("PAYPAL_EMAIL","");

	define("CATEGORY", "images/category/");
	define("CATEGORY_A", "../images/category/");
	define("CATEGORY_T", "../images/category/tempImg/");

	define("PRODUCT", "images/product/");
	define("PRODUCT_A", "../images/product/");
	define("PRODUCT_T", "../images/product/tempImg/");
?>
